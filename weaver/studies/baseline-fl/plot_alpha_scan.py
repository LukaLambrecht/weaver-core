...
if __name__=='__main__':
    ...
    # hard-coded settings
    eval_file_name = 'output.root'
    history_file_name = 'network_history_train.json'
    treename = 'Events'
    ...
    # handle case of provided result directory
      ...
      for alphadir in sorted(alphadirs):
        ...
        for subdir in sorted(os.listdir(alphadir)):
            ...
            # find result file
            eval_file = os.path.join(subdir, eval_file_name)
            if not os.path.exists(eval_file):
                print('WARNING: {} does not exist, skipping.'.format(eval_file))
                continue

            # read events
            events = get_events_from_file(eval_file,
                       treename = treename,
            ...
            # alternatively, get distance correlation from last epoch of training
            # (but not split in categories or variables)
            #history_file = os.path.join(subdir, history_file_name)
            #if not os.path.exists(history_file):
            #    print('WARNING: {} does not exist, skipping.'.format(history_file))
            #    continue
            #with open(history_file, 'r') as f:
            #    history = json.load(f)
            #disco = history[-1]['DisCo']
            #disco = [d for d in disco if not (np.isnan(d) or math.isnan(d))]
            #dccoeff = None
            #if len(disco)>0: dccoeff = np.mean(np.array(disco))
            #for cat_name in all_categories.keys():
            #    dccoeffs[cat_name] = {}
            #    for var_name in correlation_variables.keys():
            #        dccoeffs[cat_name][var_name] = dccoeff
            #if dccoeff is None: continue
      ...
      eval_file = os.path.splitext(args.outputfile)[0]+ '.json'
    ...
    for sig_name, sig_settings in signal_categories.items():
        for bkg_name, bkg_settings in background_categories.items():
            ...
            ax.plot(alphas, aucs[key]['avg'], linestyle='--', color=color, label=label)
