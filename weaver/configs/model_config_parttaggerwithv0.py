...
from weaver.nn.model.ParticleTransformer import ParticleTransformerTaggerWithV0


class ParticleTransformerTaggerWithV0Wrapper(torch.nn.Module):
    # see e.g. here:
    ...
    def __init__(self, **kwargs) -> None:
        super().__init__()
        self.model = ParticleTransformerTaggerWithV0(**kwargs)

    @torch.jit.ignore
    def no_weight_decay(self):
        return {'mod.cls_token', }

    def forward(self, pf_points, pf_features, pf_vectors, pf_mask,
                      sv_points, sv_features, sv_vectors, sv_mask,
                      v0_points, v0_features, v0_vectors, v0_mask):
        return self.model(pf_features, pf_v=pf_vectors, pf_mask=pf_mask,
                          sv_x=sv_features, sv_v=sv_vectors, sv_mask=sv_mask,
                          v0_x=v0_features, v0_v=v0_vectors, v0_mask=v0_mask)
...
def get_model(data_config, **kwargs):
    ...
    v0_features_dims = len(data_config.input_dicts['v0_features'])
    pf_points_dims = len(data_config.input_dicts['pf_points'])
    sv_points_dims = len(data_config.input_dicts['sv_points'])
    v0_points_dims = len(data_config.input_dicts['v0_points'])
    ...
    print(f'Found following V0 input feature dims: {v0_features_dims}')
    ...
    print(f'Found following V0 point dims: {v0_points_dims}')
    ...
    # get model
    model = ParticleTransformerTaggerWithV0Wrapper(**cfg)
