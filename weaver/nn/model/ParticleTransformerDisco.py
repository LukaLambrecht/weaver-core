'''
Particle Transformer with DisCo

Originally copied from nn/model/ParticleTransformer.py in this repository.
...
class ParticleTransformerDisco(nn.Module):
    ...
    def __init__(self,
        ...
        # make configuration for "block" (what is this?)
        cfg_block = copy.deepcopy(default_cfg)
        if block_params is not None:
            cfg_block.update(block_params)
        _logger.info('cfg_block: %s' % str(cfg_block))

        # make configuration for "cls_block" (what is this?)
        cfg_cls_block = copy.deepcopy(default_cfg)
        if cls_block_params is not None:
            cfg_cls_block.update(cls_block_params)
        _logger.info('cfg_cls_block: %s' % str(cfg_cls_block))

        # make embedding for individual instances and pairs of instances
        self.pair_extra_dim = pair_extra_dim
        self.embed = nn.Identity()
        if len(embed_dims) > 0: self.embed = Embed(input_dim, embed_dims, activation=activation)
        ...
        # make lists of blocks and cls_blocks
    ...
    def loss(self, predictions, labels, mass):
        ...
        # loss = cross-entropy + alpha * distance correlation(predictions, mass)
        # note: predictions, labels, and mass must be 1D tensors of the same length.
        # update: mass can be a 2D tensor of shape (number of instances, number of variables)
        ...
        # calculate distance correlation
            ...
            discoloss = sum([distance_correlation(predictions_column, mass[:,idx], power=self.disco_power)
                for idx in range(mass.shape[1])])
        else:
            msg = f'Mass tensor has unrecognized shape: {mass.shape}'
            raise Exception(msg)
    ...
    def train_single_epoch(self, train_loader, dev, **kwargs):
        ...
        with tqdm.tqdm(train_loader) as tq:
            for X, y, _ in tq:
                ...
                if not 'vectors' in data_config.input_names:
                    msg = 'Inputs specified in data config must contain the key'
                    msg += ' "vectors" for the input point vectors.'
                    raise Exception(msg)
                vectors = X['vectors'].to(dev)
                ...
                if not 'decorrelate' in data_config.input_names:
                    msg = 'Inputs specified in data config must contain the key'
                    msg += ' "decorrelate" for the decorrelation variable(s).'
                    raise Exception(msg)
                ...
                # forward pass
                predictions = self.forward(features, v=vectors, mask=mask)
