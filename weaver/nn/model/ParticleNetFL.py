...
from weaver.utils.flatnessloss import flatnessloss
...
class ParticleNetFL(nn.Module):
    ...
    def __init__(self,
        ...
        super(ParticleNetFL, self).__init__(**kwargs)
        ...
        # initializations for flatness loss method
        self.flatloss_prediction_index = flatloss_prediction_index
        self.flatloss_bins = flatloss_bins
        if flatloss_bins!='auto': self.flatloss_bins = torch.from_numpy(np.array(flatloss_bins, dtype=np.float32))
        self.flatloss_alpha = flatloss_alpha
        self.flatloss_label = flatloss_label
    ...
    def loss(self, predictions, labels, mass):
        ...
        # loss = cross-entropy + alpha * flatness loss
        ...
        # prepare data for calculation of flatness loss
        # note: can only take one dimension in the prediction!
        predictions_column = predictions[:, self.flatloss_prediction_index]

        # optional: do preselection on which predictions/mass values to use in the flatness loss
        mask = torch.ones(len(predictions_column)).cuda()
        # (todo: the cuda() option seems to be needed when running on GPU,
        #  but will probably break when running on CPU; to generalize...)
        if self.flatloss_label is not None: mask = torch.where(labels != self.flatloss_label, 0, mask)
        mask = mask.bool()
        predictions_column = predictions_column[mask]
        mass = mass[mask]

        # make bins
        if self.flatloss_bins=='auto':
            bins = torch.linspace(torch.amin(predictions_column), torch.amax(predictions_column), 5)
        else: bins = self.flatloss_bins.detach().clone()

        # calculate flatness loss
        flatloss = flatnessloss(predictions_column, mass, bins)

        # make the sum
        if( math.isnan(flatloss.item()) or np.isnan(flatloss.item()) ):
            totalloss = (1 + self.flatloss_alpha) * celoss
        else: 
            totalloss = celoss + self.flatloss_alpha * flatloss
        return (totalloss, {'total': totalloss.item(), 'BCE': celoss.item(), 'flatloss': flatloss.item()})
        ...
        with tqdm.tqdm(train_loader) as tq:
            for X, y, _ in tq:
                ...
                tq.set_postfix({
                  ...
                  'flatloss': '{:.3f}'.format(lossvalues['flatloss'])
                })
