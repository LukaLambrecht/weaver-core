...
class ParticleTransformerTaggerWithV0(nn.Module):
    ...
    def forward(self, pf_x, pf_v=None, pf_mask=None,
                      sv_x=None, sv_v=None, sv_mask=None,
                      v0_x=None, v0_v=None, v0_mask=None):

        with torch.no_grad():
            pf_x, pf_v, pf_mask, _ = self.pf_trimmer(pf_x, pf_v, pf_mask)
            sv_x, sv_v, sv_mask, _ = self.sv_trimmer(sv_x, sv_v, sv_mask)
            v0_x, v0_v, v0_mask, _ = self.sv_trimmer(v0_x, v0_v, v0_mask)
            v = torch.cat([pf_v, sv_v, v0_v], dim=2)
            mask = torch.cat([pf_mask, sv_mask, v0_mask], dim=2)

        with torch.amp.autocast('cuda', enabled=self.use_amp):
            pf_x = self.pf_embed(pf_x) # after embed: (seq_len, batch, embed_dim)
            sv_x = self.sv_embed(sv_x)
            v0_x = self.v0_embed(v0_x)
            x = torch.cat([pf_x, sv_x, v0_x], dim=0)
